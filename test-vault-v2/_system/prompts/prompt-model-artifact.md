...
6) INLINE WIKILINKS (KNOWLEDGE GRAPH)
Insert `[[wikilinks]]` inline for:
- **Named entities** (people, frameworks, methods): e.g., "concept from [[Timothy Gallwey]]"
- **Technical terms** mentioned in Source Text: e.g., "reducing [[Interference]]"
- **Related concepts** that exist or should exist in the vault: e.g., "through the [[Inner Game Equation]]"

Do NOT link generic words. Only link domain-specific terms that warrant their own note.

7) LINK CANDIDATES SECTION
After the Links section, add a "Link candidates" section listing terms that MIGHT deserve notes but are uncertain.
Format each candidate as: `[[Term]] reason:tag` where tag is one of:
